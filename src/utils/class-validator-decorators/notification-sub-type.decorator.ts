    ...
    registerDecorator({
      ...
      validator: {
        validate(value: string, args: ValidationArguments) {
          const PrimaryNotificationType = (args.object as any)?.PrimaryType;
          notificationSubtypeValidatorLogger.debug(PrimaryNotificationType);
          ...
          switch (PrimaryNotificationType) {
            case NOTIFICATION_TYPES.ARTICLE:
              const validArticleNotificationSubTypes = Object.values(
                ARTICLE_NOTIFICATION_TYPES,
              );
