...
export class GroupUsersService {
    ...
    async toggleJoinSocketRoom(
        ...
        this.groupUsersLogger.debug(
            `Joining user ${userId} to group ${groupId}`,
        );
        ...
        return await this.prismaService.$transaction(async (prisma) => {
            ...
            if (!isInRoom) {
                const messagesAvailableToRead =
                    updatedGroupUser.group.message.filter((message) => {
                        return (
                ...
                const readMessages: MessageReadReceipt[] =
                    messagesAvailableToRead.map((message) => {
                        return {
