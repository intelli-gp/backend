...
export class SearchService {
    ...
    async searchUsers(
        ...
        this.SearchLogger.log(
            `Initiate users search query about "${searchTerm}"`,
        );
    ...
    async searchArticles(searchTerm: string, from: number, size: number) {
        ...
        let numOfMatches =
            (queryResult.hits.total as SearchTotalHits).value || 0;
    ...
    async searchGroups(searchTerm: string, from: number, size: number) {
        this.SearchLogger.log(
            `Initiate groups search query about "${searchTerm}"`,
        );
    ...
    async autocomplete(
        ...
        this.SearchLogger.log(
            `Initiate suggestion query on type ${suggestType}`,
        );
