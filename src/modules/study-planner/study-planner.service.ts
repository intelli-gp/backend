...
export class StudyPlannerService {
    ...
    async updateTask(
        ...
        const { StartDate, DueDate, Title, Description, Status } =
            updateTaskDto;
    ...
    private async checkValidDate(id: number, startDate: Date, dueDate: Date) {
        ...
        for (const task of tasks) {
            ...
            if (start_date >= taskStartDate && start_date < taskDueDate)
                throw new BadRequestException({
                    message:
                        'start cannot be inside the interval of other tasks',
                });
            ...
            if (due_date >= taskStartDate && due_date <= taskDueDate)
                throw new BadRequestException({
                    message:
                        'end date cannot be inside the interval of other tasks',
                });
